...
@Component({
  selector: 'app-teams-to-do',
  templateUrl: './teams-to-do.component.html',
  styleUrls: ['./teams-to-do.component.scss']
})
export class TeamsToDOComponent implements OnInit {
  ...
  constructor(private datePipe: DatePipe, private authenticationService: AuthenticationService, private inputService: InputService, private commonEnquiryService: CommonEnquiryService, private workItemService: WorkItemService, private router: Router, private tableService: TableService, private notif: NotificationsService) {
    this.opens = 'right';
    this.drops = 'down';
  }
  ...
  async ngOnInit() {
    ...
    console.log('InputService: ', InputService.input)
    if (InputService.input !== undefined) {
      ...
      this.commonEnquiryService.getBranch(this.userIndex).subscribe(data => {
        console.log('inside enquiry oninit', data)
        if (data['result'] !== '' && data['result'] !== undefined && data['result'] != null) {
          console.log('inside onit', data['result'][0].BranchCode)
          this.branchCode = data['result'][0].BranchCode
          console.log('inside onit', this.branchCode)
  ...
  getCommonEnquiryField(branchCode, productCode) {
    this.tableService.getUserRoleDetails(this.userIndex).subscribe(data => {
      console.log('user role details', data)
        ...
        if (this.myTodoDataMap[i].LockStatus === 'N') {
          ...
          if (this.transactionRefNo !== '' && this.transactionRefNo !== undefined) {
            this.workItemService.unlockWorkItem(this.transactionRefNo, this.myTodoDataMap[i].WorkItemId).subscribe(data => {
              console.log('unloock workitem', data)
              this.unlockMsg = data[0]['result']
              if (this.unlockMsg === 'Workitem Unlocked Succesfully') {
                this.transactionRefNo = ''
                this.notif.success(
                  'Success',
                ...
                this.notif.error(
                  'Error',
                  AppConfigServiceService.settings.ErrorMessages.UnlockWorkItem,
                  {
                    timeOut: 5000,
                    showProgressBar: false,
                    pauseOnHover: true,
  ...
  loadDetails(state) {
    console.log('###search field value: ', this.value)
    console.log('load details', this.branchCode)
    if (this.branchCode !== undefined && this.branchCode !== '' && this.branchCode !== null) {
      this.loading = true;
      this.dynamicID = []
  ...
  private getMyToDoList() {
    this.tableService.getMyToDoList(this.obj).subscribe(data => {
  ...
  addToWatchList() {
    ...
    console.log(AppConfigServiceService.settings.WarningMessages.AddToWatchList, "message");
    if (this.transactionRefNo !== '' && this.transactionRefNo !== undefined) {
      this.tableService.addToWatchList(this.transactionRefNo, this.userIndex).subscribe(data => {
        ...
        this.notif.success(
          ...
          AppConfigServiceService.settings.WarningMessages.AddToWatchList,
          {
            timeOut: 5000,
            showProgressBar: false,
            pauseOnHover: true,
  ...
  clearSearchField() {
    console.log('** TeamsTODO before inside clear search field: ', this.value, " this.obj: ", this.obj)
    if (this.value !== "" && this.value !== undefined
    ...
    console.log('*** TeamsTODO after inside clear search field: ', this.value)
  }
}
